...
ActsExamples::TrackParamsLookupValidation::execute(
    const AlgorithmContext& ctx) const {
        auto particles = m_inputParticles(ctx);
        auto hits = m_inputSimHits(ctx);

        auto calibrator = [&, this](
            const Acts::GeometryContext& gctx, 
            const ActsExamples::IndexSourceLink& isl)
            -> Acts::Vector2 {
                auto hit = hits.nth(isl.index());
                return m_cfg.refLayers.at(isl.geometryId())->globalToLocal(
                    gctx, 
                    hit->position(), 
